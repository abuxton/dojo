...
Welcome to Day 10, maintained by github name.
...
This problem is modeled as solving a system of linear equations over GF(2) (binary field):

- Each machine's configuration represents a linear system where:
  - Variables = button presses (binary: pressed or not)
  - Equations = light states (each light's final state depends on which buttons toggle it)
  - Operations are in GF(2) (XOR/toggle semantics)

The solver uses **Gaussian elimination over GF(2)** to find a solution, then counts the minimum button presses (Hamming weight of solution vector).
...
day10/
├── src/
│   ├── lib.rs        # Public API: solve_part1()
│   ├── parser.rs     # Parse machine configurations
│   ├── solver.rs     # GF(2) Gaussian elimination
│   └── main.rs       # CLI runner
...
```bash
# Create example.txt with the sample input
cat > example.txt << 'EOF'
[.##.] (3) (1,3) (2) (2,3) (0,2) (0,1) {3,5,4,7}
[...#.] (0,2,3,4) (2,3) (0,4) (0,1,2) (1,2,3,4) {7,5,12,7,2}
[.###.#] (0,1,2,3,4) (0,3,4) (0,1,2,4,5) (1,2) {10,11,11,5,10,5}
...
```bash
cargo run --release
# Reads from input.txt by default
```

### Performance

```bash
# Run optimized build for better performance
cargo build --release
...
- [x] Implement parser for machine configurations
- [x] Implement GF(2) Gaussian elimination solver
- [x] Add unit tests for example input
- [x] Create modular structure (lib, parser, solver, main)
- [ ] Add puzzle input file
- [ ] Validate against actual puzzle input
- [ ] Update README with final answer
